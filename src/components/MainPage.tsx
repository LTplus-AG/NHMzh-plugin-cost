      ...
      try {
        ...
        if (elements && elements.length > 0) {
          ...
          const getElementQuantity = (
            ...
            if (el.quantity && typeof el.quantity === "object") {
              return { value: el.quantity.value || 0, unit: el.quantity.unit };
            }

            const record = el as Record<string, unknown>;

            if (typeof record.area === "number") {
              return { value: record.area, unit: "m²" };
            }
            ...
            if (typeof record.length === "number") {
              return { value: record.length, unit: "m" };
            }

            if (el.quantity_value !== undefined) {
              return { value: el.quantity_value };
            }

            if (typeof record.original_area === "number") {
              return { value: record.original_area, unit: "m²" };
            }

            if (el.original_quantity?.value !== undefined) {
              return {
                value: el.original_quantity.value,
                unit: el.original_quantity.unit,
              };
            }
          ...
          elements.forEach((el: MongoElement) => {
            ...
            const qInfo = getElementQuantity(el);
            if (!statMap[code]) statMap[code] = { quantity: 0, unit: qInfo.unit };
            statMap[code].quantity += qInfo.value;
            if (!statMap[code].unit && qInfo.unit) statMap[code].unit = qInfo.unit;
          });
